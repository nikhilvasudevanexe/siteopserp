// ════════════════════════════════════════════════════════════════
// TWEAKS PANEL — floating live-control panel (from the design starter,
// adapted to run standalone: its own launcher + local state instead of
// the design-tool host iframe protocol).
// ════════════════════════════════════════════════════════════════
...
const TWEAKS_STYLE = `
  .twk-launch{position:fixed;right:16px;bottom:16px;z-index:2147483646;
    ...
    padding:10px 14px;cursor:pointer;display:flex;align-items:center;gap:7px}
  ...
  if (!open) {
    return (
      <>
        ...
        <button className="twk-launch" onClick={() => setOpen(true)}>⚙ {title}</button>
      </>
    );
  }
  return (
  ...
  return (
    <TweakRow label={label}>
      <div className="twk-chips" role="radiogroup">
        {options.map((o, i) => {
          const on = String(o).toLowerCase() === String(value).toLowerCase();
          return (
            <button key={i} type="button" className="twk-chip" role="radio"
              aria-checked={on} data-on={on ? "1" : "0"} title={o}
